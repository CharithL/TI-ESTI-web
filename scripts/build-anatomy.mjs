#!/usr/bin/env node
/**
 * build-anatomy.mjs — builds docs/anatomy/: the Anatomy of an Abstract Theory
 * conversations from Downloads ("<name> – A Conversation.html"). Adapted from
 * build-index.mjs, whose notes below still describe the shared machinery.
 *
...
const SOURCE = argOf('--source', process.env.ANATOMY_SOURCE || 'C:\\Users\\chari\\Downloads');
const OUT    = join(ROOT, 'docs', 'anatomy');   // a third site inside the same Pages site

const RENAME = {};   // nothing to rename: every file here is a conversation

/* The reading order of the series, matched on the start of the filename. Anything
 * not listed follows these, newest first, so a new conversation still appears. */
const ORDER = [
  'Money, Gravity and the Posit',
  'The Conceptual Anatomy of Theoretical Posits',
  'Opus 4.8 on the Anatomy',
 ...
 * ------------------------------------------------------------------ */
/* Downloads is a busy folder, so the pattern is deliberately narrow: only files
 * named "<name> – A Conversation.html", with the en dash, are published. */
const ALLOW = [
  /^.+ – A Conversation\.html$/,
];
...
const ASSETS = join(ROOT, 'scripts', 'assets-anatomy');
const ART_FILE = join(ROOT, 'scripts', 'art-credits-anatomy.json');
 ...
 * on every document: Wright of Derby's Air Pump, darkened and slightly blurred so
 * the site name and the document's own artwork stay dominant. */
const BANNER_CSS = `<style>
.anat-banner{position:relative;display:block;width:100%;height:clamp(190px,27vw,320px);
margin:0 0 28px;overflow:hidden;background:#1f130a;isolation:isolate}
.anat-banner img{border:0;max-width:none}
.anat-banner .anat-bg{position:absolute;inset:0;width:100%;height:100%;object-fit:cover;object-position:50% 42%;
filter:blur(1.5px) saturate(.8) brightness(.46)}
.anat-banner .anat-fg{position:absolute;top:0;bottom:0;right:clamp(14px,7vw,110px);height:100%;width:auto;
max-width:52%;object-fit:contain;box-shadow:0 0 48px rgba(0,0,0,.55)}
.anat-banner .anat-shade{position:absolute;inset:0;
background:linear-gradient(90deg,rgba(20,11,4,.8),rgba(20,11,4,.16) 58%),
linear-gradient(0deg,rgba(20,11,4,.5),transparent 42%)}
.anat-banner .anat-home{position:absolute;z-index:2;left:clamp(16px,4.5vw,52px);top:50%;transform:translateY(-50%);
...
.anat-banner .anat-home span{color:#e8b27a;font-weight:400}
.anat-banner .anat-home:hover{color:#e8b27a}
.anat-banner .anat-cap{position:absolute;z-index:2;right:14px;bottom:9px;margin:0;max-width:62%;text-align:right;
font:400 10.5px/1.45 ui-sans-serif,system-ui,"Segoe UI",sans-serif;letter-spacing:.04em;color:rgba(247,236,218,.7)}
@media (max-width:640px){
.anat-banner{height:clamp(150px,42vw,210px)}
.anat-banner .anat-fg{max-width:60%;right:10px}
.anat-banner .anat-home{font-size:1.75rem;left:15px}
.anat-banner .anat-cap{display:none}}
@media print{.anat-banner{display:none}}
</style>`;
...
<div class="anat-banner">
  ...
  <div class="anat-shade"></div>
  <a class="anat-home" href="./" title="Anatomy — all conversations">Anatomy</a>
  <p class="anat-cap">${esc(art.caption)}</p>
</div>`;
...
const GROUPS = [
  { key: 'origin', test: n => /^Money, Gravity/i.test(n), label: 'Where it began',
...
function meta(html, filename) {
   ...
   * " – A Conversation", and on two pages the two differ outright. */
  const nm = html.match(/<span class="nm">([\s\S]{0,160}?)<\/span>/i);
  let title = nm ? dec(strip(nm[1]))
            : (t ? dec(strip(t[1])) : basename(filename, extname(filename)));
  title = title.replace(/\s*[–-]\s*A Conversation\s*$/i, '');
...
function slug(name) {
  return basename(name, extname(name))
    .replace(/^_/, '')
    .replace(/\s*[–-]\s*A Conversation\s*$/i, '')   // every file carries this suffix
...
function collect() {
  ...
  /* the series has a reading order; anything not named in ORDER follows, newest first */
  const rank = it => { const i = ORDER.findIndex(p => it.name.startsWith(p)); return i < 0 ? ORDER.length : i; };
...
<title>Anatomy · of an abstract theory</title>
<meta name="description" content="Anatomy of an Abstract Theory — conversations on what a theory's unseen posits are made of: money against gravity, the architecture shared by physics and the humanities, what predictive success does and does not earn, and the vat tower.">
...
<header class="hero">
  <div class="hero-img" role="img" aria-label="Joseph Wright of Derby, A Philosopher Lecturing on the Orrery"></div>
  <div class="hero-in">
    <div class="hero-title">
      <h1>Anatomy</h1>
      <p class="gloss">Of an abstract theory <i>what is a posit made of?</i></p>
    </div>
  </div>
  <p class="credit">Joseph Wright of Derby, <i>A Philosopher Lecturing on the Orrery</i>, 1766</p>
</header>
...
<footer class="site-foot"><div class="in">
  <p><b>Anatomy</b> &middot; of an abstract theory. Generated ${new Date().toISOString().slice(0, 10)} &middot; ${total} pages. Questions are the author&rsquo;s, corrected for grammar and spelling only; answers are reproduced as given.</p>
  <p>Also here: <a href="../plato/"><b lang="grc">τί ἐστι;</b></a> &mdash; Plato and the Socratic Question, <a href="../descartes/"><b>Cogito?</b></a> &mdash; Descartes, and <a href="../">the Lyceum</a>.</p>
  <p>Banner: Joseph Wright of Derby, <i>A Philosopher Lecturing on the Orrery</i> (1766) &mdash; public domain, via <a href="https://commons.wikimedia.org/wiki/File:Wright_of_Derby,_The_Orrery.jpg">Wikimedia Commons</a>.</p>
  <p>Behind the banner on every page: Joseph Wright of Derby, <i>An Experiment on a Bird in the Air Pump</i> (1768) &mdash; public domain.</p>
